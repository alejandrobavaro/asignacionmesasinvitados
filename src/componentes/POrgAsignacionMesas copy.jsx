...
// Componente principal para la organización de mesas
function POrgAsignacionMesas() {
  // Estados para la gestión de invitados y mesas
  const [invitadosData, setInvitadosData] = useState(null); // Datos de invitados
  const [tables, setTables] = useState([]); // Lista de mesas
  const [searchTerm, setSearchTerm] = useState(''); // Término de búsqueda
  const [activeGroup, setActiveGroup] = useState(null); // Grupo activo
  const [selectedTable, setSelectedTable] = useState(null); // Mesa seleccionada

  // Estados para la creación de nuevas mesas
  const [nuevaMesa, setNuevaMesa] = useState({ nombre: '', capacidad: 8 }); // Nueva mesa a crear
                              ...
                              {(provided, snapshot) => (
                                ...
                                >
                                  <div className="invitado-detalle">
                                    ...
                                    {!snapshot.isDragging && <span className="invitado-grupo-mesa">{guest.grupo}</span>}
                                  </div>
                                  {!snapshot.isDragging && (
                                    <button
